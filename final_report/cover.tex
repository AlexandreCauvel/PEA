...
The $R_{\text{s}}$, also known as ohmic or uncompensated resistance, is the resistance resulting from the potential drop between the working electrode and the reference electrode \cite{MetrohmEIS3, Lasia2014}. Its value depends on the specific conductivity of the bulk electrolyte and the geometry of the electrode \cite{MetrohmEIS3}. In electrochemical impedance spectroscopy (EIS), $R_{\text{s}}$ is found on a Nyquist plot at the intersection of the impedance data with the real axis at the high-frequency end. 

$R_{\text{ct}}$ is the real component of faradaic impedance that dictates the transfer of electrons between an electrode and the electroactive species in a solution \cite{Lasia2014}. Sometimes referred to as polarization resistance in Faradaic electrochemical processes, its value is strictly dependent on reaction kinetics and is inversely proportional to the heterogeneous rate constants of the electrode reactions \cite{Lasia2014,Das2025}. In electrochemical impedance spectroscopy (EIS) measurements, $R_{\text{ct}}$ typically defines the diameter of the high-frequency semicircle observed on a Nyquist plot, a feature that results from its coupling with the double-layer capacitance \cite{Das2025,Lasia2014}.

The $C_{\text{dl}}$ is the capacitance that inherently forms at the electrode/electrolyte interface \cite{Kelly2002,MetrohmEIS3}. This electrical double layer develops when ions from the electrolytic solution approach the electrode surface, creating a separation between the charges in the solution and the charges within the electrode as described in section [REFERENCIAR SECÇÃO DA DOUBLE LAYER] \cite{MetrohmEIS3}. While liquid electrodes can act as ideal capacitors, solid electrodes rarely exhibit pure capacitive behavior and instead show frequency dispersion \cite{Lasia2014}. This non-ideal behavior is caused by surface heterogeneities, roughness, porosity, and non-uniform current distributions \cite{Das2025}. To accurately account for this dispersion, the double layer at solid electrodes is most commonly modeled using a Constant Phase Element (CPE) rather than an ideal capacitor \cite{Das2025,Lasia2014,MetrohmEIS3}.

It also becomes important to define time constants. A time constant ($\tau$) define the characteristic duration required for a specific electrochemical process to relax from a transient state to a stable one \cite{Das2025}. In electrical equivalent circuits, a time constant is calculated as the product of a resistance and a capacitance ($\tau = RC$.) and dictates the rate at which the system's current or voltage responds to a perturbation \cite{Lasia2014}. Because an electrochemical cell involves multiple parallel and sequential processes—such as charge transfer, ion diffusion, and adsorption—each fundamental process is guided by its own distinct time constant \cite{Das2025}. For example, the kinetic time constant of an interface is defined as $\tau_{k} = R_{\text{ct}} C_{\text{dl}}$ \cite{Lasia2014}. In frequency-domain measurements like Bode plots, these time constants correspond to "break-point" or characteristic frequencies ($\omega = \frac{1}{RC}$), which mark the transition between different regimes, such as moving from a capacitive-controlled response to a diffusion-dominated response \cite{Lasia2014,Das2025}.

Charge transfer resistance is inversely proportional to the corrosion rate \cite{Kelly2002}. According to the Stern-Geary relationship, the instantaneous corrosion current is calculated by dividing a constant (derived from the anodic and cathodic Tafel slopes) by the $R_{\text{ct}}$ \cite{Kelly2002}. Consequently, a higher resistance indicates slower electrochemical reaction kinetics and better corrosion protection \cite{Kelly2002}.

Conversely, an increase in $C_{\text{dl}}$ is indicative of protective film degradation: as the film breaks down, a larger area of bare metal is exposed directly to the corrosive electrolyte, increasing the effective capacitive area \cite{Kelly2002}. Furthermore, as localised corrosion progresses, for instance through pitting, the electrode surface becomes increasingly rough and geometrically complex, further enlarging the effective interfacial area and driving $C_{\text{dl}}$ to higher values \cite{Lasia2014,Kelly2002}. This dependency can be understood through the expression for a planar capacitor:
