...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{} l c c @{}}
        \toprule
        Element & \makecell{Norm. at.\,(\%) \\ (Al matrix)} & 
...
Crucially, the Zn content is nearly identical across both regions --- 8.79\,at.\,\% over the Al matrix and 8.25\,at.\,\% over the Si intermetallic. Since Zn is absent from the AC46000 alloy and is incorporated exclusively through the synthesis solution, its uniform distribution across both substrate types provides strong and direct evidence that LDH was deposited with comparable density over both the aluminium matrix and the silicon intermetallic particles.

Further insight is obtained from the Zn:Al atomic ratios. Over the aluminium matrix, the measured Zn:Al ratio is approximately 0.32; however, in this region the Al signal contains contributions from both the LDH film and the underlying aluminium substrate, which inflates the Al denominator and consequently underestimates the true Zn:Al ratio of the LDH film alone. Over the silicon intermetallic particles, by contrast, the substrate contributes no Al signal, meaning the measured Al derives exclusively from the LDH film itself. The resulting Zn:Al ratio of approximately 1.15 therefore more faithfully reflects the intrinsic stoichiometry of the deposited LDH phase. Considering that the theoretical Zn:Al ratio in ZnAl--LDH corresponds to the $x$ parameter in the general formula, typically in the range $x = 0.25$--$0.33$ \cite{Zhou2017}, which for a Zn$_{1-x}$Al$_x$ composition yields a Zn:Al ratio of approximately 2:1 to 3:1, the experimentally determined ratio over the Si intermetallic is broadly consistent with a well-formed ZnAl--LDH phase. The elevated carbon content detected in both regions is attributed to adventitious surface contamination, commonly observed in EDS analyses conducted under ambient conditions.
...
\subsection{LDH-1: Industrial Pre-treatment, 1H Growth}
\label{subsec:ldh1}
...
\subsection{LDH-2: Chemical Pre-treatment, 3H Growth}
\label{subsec:ldh2}
...
%ADD PICTURES AT 72H FOR LDH-2
%ADD POLARIZATION CURVES AND TALK ABOUT THEM
%ADD PICTURES OF TOPCOATINGS AND COMPLEX GEOMETRIES AND TALK ABOUT THEM
