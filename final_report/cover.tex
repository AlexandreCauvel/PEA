...
\begin{figure}[htbp]
    ...
    \caption{XRD patterns of LDH films grown on specimens subjected to the four substrate conditions. Reference patterns for the aluminium matrix and silicon intermetallic phases are included for comparison. The inset shows the low-angle region highlighting the (003) and (006) basal reflections characteristic of the ZnAl--LDH phase after baseline subtraction.}
    \label{fig:xrd_ldh}
\end{figure}
...
Further insight is obtained from the Zn:Al atomic ratios. Over the aluminium matrix, the measured Zn:Al ratio is approximately 0.32; however, in this region the Al signal contains contributions from both the LDH film and the underlying aluminium substrate, which inflates the Al denominator and consequently underestimates the true Zn:Al ratio of the LDH film alone. Over the silicon intermetallic particles, the substrate contributes no Al signal from the Si phase itself, meaning the measured Al derives predominantly from the LDH film. However, the resulting Zn:Al ratio of approximately 1.15, while higher than that measured over the aluminium matrix (0.32), remains below the theoretical Zn:Al ratio of ZnAl--LDH of approximately 2:1 to 3:1 --- corresponding to the $x$ parameter in the general formula, typically in the range $x = 0.25$--$0.33$ \cite{Zhou2017}. This suggests that the Al signal over the Si intermetallic is still partially inflated by a residual contribution from the surrounding aluminium matrix, captured within the lateral spread of the EDS interaction volume at the particle boundaries. Nonetheless, the progressive increase in the Zn:Al ratio from the matrix region to the Si intermetallic region is consistent with a reduced substrate Al contribution and supports the conclusion that a ZnAl--LDH film of comparable density was deposited over both substrate types. The carbon signal detected in both regions is attributed to the conductive carbon film deposited on the specimens prior to SEM-EDS analysis, a standard sample preparation procedure employed to prevent surface charging during electron beam irradiation.
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Corrosion potential ($E_{\text{corr}}$) and corrosion current 
    density ($i_{\text{corr}}$) extracted from potentiodynamic polarisation 
    curves for LDH-2 specimens following \qty{168}{\hour} of immersion in 
    \qty{0.05}{\molar} \ce{NaCl}.}
    \label{tab:corrosion_parameters}
    \begin{tabular}{@{} l c c @{}}
        \toprule
        \makecell[l]{Sample} & 
        \makecell{$E_{\text{corr}}$ (V vs. Ref.)} & 
        \makecell{$i_{\text{corr}}$ (A/cm$^{2}$)} \\
        \midrule
        Control        & $-0.596$ & $2.446 \times 10^{-5}$ \\
        \ce{NaVO3}     & $-0.512$ & $1.189 \times 10^{-5}$ \\
        MBT            & $-0.519$ & $1.165 \times 10^{-5}$ \\
        \bottomrule
    \end{tabular}
\end{table}

The potentiodynamic polarisation curves for the LDH-2 specimens following \qty{168}{\hour} of immersion are presented in Figure~\ref{fig:pdp}, and the extracted corrosion parameters are summarised in Table~\ref{tab:corrosion_parameters}. Given the diffusion-limited nature of the cathodic reaction observed in all specimens --- manifested as a pronounced mass-transport limiting current plateau in the cathodic branch --- conventional Tafel extrapolation was not applicable. Instead, the mass-transport corrected Butler--Volmer equation was employed to extract $E_{\text{corr}}$ and $i_{\text{corr}}$ from the experimental curves.

Inspection of the polarisation curves reveals that the cathodic branches of all three specimens are virtually superimposed, indicating that the cathodic reaction rate is essentially identical regardless of the inhibitor system employed. Since the cathodic reaction in this system is driven primarily by oxygen reduction at cathodic intermetallic sites --- principally Fe--Mn and Cu-rich particles, as discussed in Section~\ref{sec:results_pretreatments} --- this observation confirms that neither \ce{NaVO3} nor MBT is effective at suppressing the activity of these cathodic sites under the conditions investigated. In contrast, a clear differentiation is observed in the anodic branches: the inhibitor-intercalated specimens exhibit a measurably lower anodic current density than the control, indicating that while the inhibitors do not suppress cathodic intermetallic activity, they do retard anodic dissolution of the aluminium matrix to a significant extent. This is corroborated quantitatively by the $i_{\text{corr}}$ values in Table~\ref{tab:corrosion_parameters}: the control specimen exhibits an $i_{\text{corr}}$ of $2.446 \times 10^{-5}$~A~cm$^{-2}$, approximately twice that of the \ce{NaVO3} ($1.189 \times 10^{-5}$~A~cm$^{-2}$) and MBT ($1.165 \times 10^{-5}$~A~cm$^{-2}$) systems, confirming a meaningful reduction in the overall corrosion rate conferred by both inhibitor systems. The comparable $E_{\text{corr}}$ values of the \ce{NaVO3} and MBT specimens, both shifted approximately \qty{80}{\milli\volt} in the noble direction relative to the control, are consistent with partial anodic inhibition reducing the equilibrium between anodic and cathodic partial reactions.

While the LDH-2 batch represents a clear improvement over LDH-1, the overall corrosion protection performance remains below the target level. This is attributed primarily to the persistence of Fe--Mn intermetallic particles on the chemically pre-treated surface, as evidenced in Section~\ref{sec:results_pretreatments}. Unlike Cu-rich particles, which are the primary targets of vanadate and MBT inhibitors \cite{Tedim2011, Zhang2015}, the Fe--Mn IMPs characteristic of AC46000 are not effectively passivated by either inhibitor system under the conditions investigated, and their continued activity as cathodic sites sustains localised corrosion that the inhibitor loading achievable within the LDH interlayer galleries is insufficient to fully suppress.

\section{Top Coating and Complex Geometries}
\label{sec:results_topcoat}

Following the successful deposition of the inhibitor-intercalated LDH conversion coating, a water-based polyurethane colloidal formulation was applied to both planar and geometrically complex specimens via anodic electrodeposition, as described in Section~\ref{sec:topcoat}. The resulting coating systems were evaluated by optical inspection, and representative photographs of both planar and complex geometry specimens are presented in Figure~\ref{fig:optical_topcoat}.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figs/topcoat/planar_MBT.jpg}
        \caption{Planar MBT 1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figs/topcoat/cg_MBT_1.jpg}
        \caption{Complex shape MBT 1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figs/topcoat/cg_MBT_2.jpg}
        \caption{Complex shape MBT 2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figs/topcoat/planar_NaVO3.jpg}
        \caption{Planar \ce{NaVO3} 1}
        \label{fig:topcoat_planar_NaVO3}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figs/topcoat/cg_NaVO3_1.jpg}
        \caption{Complex shape \ce{NaVO3} 1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figs/topcoat/cg_NaVO3_2.jpg}
        \caption{Complex shape \ce{NaVO3} 2}
    \end{subfigure}
    \caption{Optical photographs of polyurethane-coated specimens following anodic electrodeposition and curing. Top row (a)--(c): MBT-intercalated specimens. Bottom row (d)--(f): \ce{NaVO3}-intercalated specimens. Left column: planar substrates. Centre and right columns: geometrically complex substrates.}
    \label{fig:optical_topcoat}
\end{figure}

Optical inspection of the coated specimens, presented in Figure~\ref{fig:optical_topcoat}, confirms that the polyurethanetopcoat forms a uniform and continuous film across all specimens, for both planar and geometrically complex substrates, demonstrating that the anodic electrodeposition process is capable of conformally coating non-planar geometries.The cross-cut adhesion test additionally confirmed the absence of delamination at the cut edges, indicating good interfacial adhesion between the polyurethane topcoat and the underlying LDH conversion coating. One notable observation isthe presence of surface pores visible on the planar \ce{NaVO3} specimen (Figure~\ref{fig:topcoat_planar_NaVO3}). These are attributed topre-existing macroscopic porosity in the die-cast substrate rather than to the deposition process itself: the casting process inherently generates gas porosity and shrinkage voids within the material, and where these defects intersect the surface they present cavities of sufficient size that the polyurethane film is unable to bridge them completely, leaving residual open pores in the final coating.
